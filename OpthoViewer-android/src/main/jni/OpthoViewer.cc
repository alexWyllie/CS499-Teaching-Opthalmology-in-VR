...
namespace OpthoViewer {
    ...
    namespace {
        ...
        // Set the location of the object to always be in the same place
        // beside and close enough to the user to fill the whole display
        ...
        // Threshold here is used to determine how close to an invisible
        // feature the line of site is for determining the annotations
        constexpr float kAngleLimit = 0.2f;

        // Number of possible objects o render when doing randomized quizzing
   ...
   void main() {
    ...
    // The OpthosApp manages the running of the app
    OpthosViewer::OpthosApp(JavaVM* vm, jobject obj, jobject asset_mgr_obj)
    ...
    // Class destructor for ending the session
    OpthosViewer::~OpthosApp() {
        CardboardHeadTracker_destroy(head_tracker_);
        CardboardLensDistortion_destroy(lens_distortion_);
        CardboardDistortionRenderer_destroy(distortion_renderer_);
    }

    // Override for shading
    void OpthosViewer::OnSurfaceCreated(JNIEnv* env) {
        ...
        // These are for the unit tests / debugging
    ...
    // Set the parameters for width and height
    void OpthosViewer::SetScreenParams(int width, int height) {
        screen_width_ = width;
        screen_height_ = height;
        screen_params_changed_ = true;
    }

    // Primary frame draw loop
    void OpthosViewer::OnDrawFrame() {
    ...
    // Define what to do when the user has stopped the environment
    void OpthosViewer::OnPause() { CardboardHeadTracker_pause(head_tracker_); }

    // Define how to restart the app when
    void OpthosViewer::OnResume() {
    ...
    // Switch to new cardboard parameters
    void OpthosViewer::SwitchViewer() {
        CardboardQrCode_scanQrCodeAndSaveDeviceParams();
    }

    // Set the device to do its thing
    bool OpthosViewer::UpdateDeviceParams() {
    ...
    void OpthosViewer::GlSetup() {
        // add more of these are part of testing
        LOGD("GL SETUP");
    ...
    Matrix4x4 OpthosViewer::GetPose() {
    ...
    // Convenience function for creating the whole scene
    void OpthosViewer::DrawWorld() {
        DrawRoom();
        DrawTarget();
    }

    // This renders the eye
    void OpthosViewer::DrawTarget() {
        glUseProgram(obj_program_);
    ...
    // This renders the background (shouldn't need in final
    void OpthosViewer::DrawRoom() {
        glUseProgram(obj_program_);
